 ...
 * {@link CheckpointTrigger#RESOURCE_KEY}. A pooled streaming event processor places it there per batch, but <b>only for
 * a self-checkpointing component</b> -- one whose handler (or the POJO it wraps) implements {@link Checkpointing}.
 * <p>
 ...
 * only usable on a handler whose component implements {@link Checkpointing} and that runs in a pooled streaming event
 * processor; declaring it on a handler that may run in a subscribing or otherwise non-checkpointing processor, or on a
 * component that does not implement {@link Checkpointing}, will raise that exception at runtime.
 *
...
public class CheckpointTriggerParameterResolverFactory implements ParameterResolverFactory {
     ...
     * parameter is never silently skipped. When no trigger is present (a non-checkpointing processor, or a component
     * that does not implement {@link Checkpointing}), {@link #resolveParameterValue(ProcessingContext)} fails the
            ...
            return CheckpointTrigger.fromContext(context)
                                    ...
                                    .orElseGet(() -> CompletableFuture.failedFuture(new IllegalStateException(
                                            "Cannot resolve a CheckpointTrigger parameter: no CheckpointTrigger is "
                                                    + "present in the current ProcessingContext. A CheckpointTrigger "
                                                    + "is only available to a self-checkpointing component (one "
                                                    + "implementing Checkpointing) running in a pooled streaming event "
                                                    + "processor. Ensure the handler's component implements "
                                                    + "Checkpointing, or remove the CheckpointTrigger parameter from "
                                                    + "handlers that may run in a non-checkpointing processor.")));
        }
