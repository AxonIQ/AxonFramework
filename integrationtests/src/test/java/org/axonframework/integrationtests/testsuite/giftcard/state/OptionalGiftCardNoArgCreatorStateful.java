 ...
 * command handlers because the handler lives outside the entity and receives an empty {@link Optional} when it does not
 * exist yet.
 * <p>
 * Although the no-arg constructor could produce an empty entity without any preceding event, the {@code @InjectEntity}
 * parameter is declared as {@code Optional<GiftCard>} here, so it consistently resolves to {@link Optional#empty()}
 * when there is no event to source the entity from, just as the id-based and event-based creator styles do.
 *
 * @author Steven van Beelen
 */
public class OptionalGiftCardNoArgCreatorStateful {
