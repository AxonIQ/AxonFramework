 ...
 * <p>
 * It exists alongside the course-statistics store to show that several tenant-scoped types
...
public interface AuditLog extends AutoCloseable {
    ...
    /**
     * Creates an in-memory audit log for the tenant with the given {@code tenantId}, the implementation
     * this demo uses. Callers depend only on this interface, never on the implementation type.
     *
     * @param tenantId the identifier of the tenant the audit log belongs to
     * @return an in-memory audit log for the tenant
     */
    static AuditLog inMemory(String tenantId) {
        return new InMemoryAuditLog(tenantId);
    }
