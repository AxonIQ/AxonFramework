...
|  +- component                   the tenant-aware components: CourseStatsStore and AuditLog (+ in-memory impls)
|  +- write/enrol                 EnrolStudent and its command handler
...
+- shared                         the driving utilities both runnable demos use
   ...
   +- Enrolments                  sends the command and reads the query through the gateways
   ...
   +- TenantProvisioning          in-memory vs Axon Server tenant provisioning
   +- AxonServerTenantContexts    creates and deletes Axon Server contexts
...
1. Enrol students in the tenants known at startup, and read each tenant's statistics back to show it
   sees only its own.
2. Add a tenant at runtime, enrol into it, and show its components appear on first use.
