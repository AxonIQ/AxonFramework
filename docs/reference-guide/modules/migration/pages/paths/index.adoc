...
- **Running sagas and scheduled deadlines**: There is no `Saga` construct, `SagaStore` or `DeadlineManager` in Axon Framework 5, and no tooling to carry the contents of an Axon Framework 4 `SagaStore` or scheduled deadlines across.
  An `axon-legacy` module is being built to keep existing sagas and their deadlines running on Axon Framework 5 almost unchanged, so that long-running processes can drain after the upgrade rather than before it. It is not released yet.
