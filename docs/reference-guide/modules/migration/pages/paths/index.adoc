...
- **Saga & Deadline state migration**: There is no `Saga` construct, `SagaStore` or `DeadlineManager` in Axon Framework 5, and no tooling to carry the contents of an Axon Framework 4 `SagaStore` or scheduled deadlines across.
  New processes are fully supported today: see xref:sagas:index.adoc[Sagas and Process Managers] for the available approaches, and xref:migration:paths/sagas.adoc[Saga migration] for how to drain running sagas rather than convert them.
