...
* **Trace topology changed**: distributed commands and queries always continue the dispatcher's trace, and event handling outside the publication context uses per-event spans under a batch span with a link back to the publisher.
...
[source,java]
----
include::example$migration/paths/tracing/TracingConfiguration.java[tag=register-span-factory,indent=0]
----
...
[source,java]
----
include::example$migration/paths/tracing/springboot/TracingConfiguration.java[tag=register-span-factory-spring,indent=0]
----
====
...
[source,java]
----
include::example$migration/paths/tracing/MultipleExportersConfiguration.java[tag=multiple-exporters-configuration-api,indent=0]
----
+
...
[source,java]
----
include::example$migration/paths/tracing/springboot/MultipleExportersConfiguration.java[tag=multiple-exporters-spring,indent=0]
----
+
...
Register the customized provider through the Configuration API or expose it as a Spring bean:
...
[source,java]
----
include::example$migration/paths/tracing/AttributeProviderConfiguration.java[tag=legacy-message-id-key,indent=0]
----
...
[source,java]
----
include::example$migration/paths/tracing/springboot/AttributeProviderConfiguration.java[tag=legacy-message-id-key-spring,indent=0]
----
====
...
[source,java]
----
include::example$migration/paths/tracing/TenantSpanAttributesProvider.java[tag=provider-signature,indent=0]
----
====

The provider shares `TENANT_ID` with the interceptor or handler that stores the tenant identifier through
`ProcessingContext.putResource(...)`.
It omits the attribute when the provider runs without a processing context or when no tenant is available.
...
[source,java]
----
include::example$migration/paths/tracing/TracingConfiguration.java[tag=register-tracing-settings,indent=0]
----
+
...
* **Consumers handling outside the publication context**, including pooled streaming processors and subscribing processors fed by a Persistent Stream, now parent each `EventProcessor.process` span under the `StreamingEventProcessor.batch` span that executes it, with a span link back to the publisher. In Axon Framework 4 the batch span was a separate, childless trace. The publisher relationship stays a link because a batch can carry events from several publishers, and an event handled long after publication (catch-up, replay) must not stretch the publisher's finished trace.
