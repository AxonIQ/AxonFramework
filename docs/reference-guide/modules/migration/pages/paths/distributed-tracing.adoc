...
Axon Framework 5 with Spring Boot::
+
[source,xml]
----
<dependency>
...
This dependency set is the Spring Boot option.
A plain-Java application instead selects and configures its tracing SDK and a compatible `SpanFactory` binding in its own observability bootstrap.
...
* Exporters can be toggled per environment through the SDK's configuration. Spring Boot properties and exporter beans are one option, but fan-out is not tied to Spring.
