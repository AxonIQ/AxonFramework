...
[source,java]
----
include::example$migration/paths/sequencingpolicies/CustomerIdSequencingPolicy.java[tag=customer-id-sequencing-policy,indent=0]
----
====
...
The `HierarchicalSequencingPolicy` then falls back to `SequentialPolicy`, which answers with the same constant sequence identifier for every event.
Because a segment is chosen by hashing the sequence identifier, one identifier means one segment: every event is routed to the single segment that constant hashes into, and every other configured segment receives nothing.
Raising the segment count or the thread count therefore buys no throughput at all.
This is the safe default, but it removes parallelism entirely: see the section below on defining an explicit policy.
...
[source,java]
----
include::example$migration/paths/sequencingpolicies/annotation/OrderEventHandler.java[tag=annotation-sequencing-policy,indent=0]
----
...
[source,java]
----
include::example$migration/paths/sequencingpolicies/declarative/AxonConfig.java[tag=declarative-sequencing-policy-config,indent=0]
----
