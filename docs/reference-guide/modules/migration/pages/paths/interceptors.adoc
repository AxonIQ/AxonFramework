...
[NOTE]
====
This mirrors Axon Framework 4's behavior exactly: a plain `@Component`-annotated class implementing `MessageHandlerInterceptor` or `MessageDispatchInterceptor` is automatically discovered and registered, with no manual `@Bean` method or explicit registration call required.
The Axon Framework 5 tabs above are a complete, working Spring Boot configuration on their own.
====
