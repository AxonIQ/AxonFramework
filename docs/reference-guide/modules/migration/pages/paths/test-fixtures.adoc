...
[NOTE]
====
There is no dedicated `SagaTestFixture` in Axon Framework 5.
Process Managers are regular event-handling and command-handling components, so test them through `AxonTestFixture` using the same application configuration as production.
See the xref:saga-guide::index.adoc[Saga Pattern and Process Managers guide] for the available implementation approaches.
====
