...
To configure a policy, pass a lambda to the relevant method on `QueryGatewayConfigurer` (for gateway-level tracking) or create a `QueryShutdownManager` directly (for call-site tracking):
...
AxonConfiguration config = MessagingConfigurer.create()
    .queryGateway("sse", g -> g
        .cancellingSubscriptionQueryOnShutdown(c -> c.closeImmediately()) // <1>
        .cancellingStreamingQueryOnShutdown(c -> c.withGracePeriod(Duration.ofSeconds(5))) // <2>
    )
    .build();
...
MessagingConfigurer.create()
    ...
    .queryGateway("internal", g -> g
        .cancellingSubscriptionQueryOnShutdown(c -> c.withGracePeriod(30))
    );
----
...
<2> For direct `QueryBus` users, wrap the `MessageStream` with `track()` and use the returned wrapper. `queryMessage` must be a `QueryMessage` instance.
...
<1> Inject the `QueryShutdownManager` bean declared above. Spring's `@Qualifier` selects the right one when multiple managers are defined.
...
Inject the appropriate manager by name using `@Qualifier` at each call site or gateway declaration.
