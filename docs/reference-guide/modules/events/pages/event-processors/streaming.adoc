...
That policy does not read the aggregate identifier from the event itself, but from a resource the event store publishes on the xref:messaging-concepts:processing-context.adoc[processing context] alongside each event.
Only the aggregate-based storage engines publish that resource.
An event store speaking the Dynamic Consistency Boundary protocol does not, so there the policy resolves nothing for _every_ event, and the `HierarchicalSequencingPolicy` falls back to the `SequentialPolicy`.
That fallback answers with the same constant sequence identifier for every event, so all events are handled in the order they've been published by a **single** segment, thus eliminating any possibility for parallel processing.
Every other configured segment receives nothing, which means raising the segment count or the thread count buys no throughput while this fallback is in effect.
