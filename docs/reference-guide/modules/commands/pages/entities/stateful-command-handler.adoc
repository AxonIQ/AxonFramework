...
The same works for a `ManagedEntity` parameter: declare it as `Optional<ManagedEntity<ID, GiftCard>>` to get an empty `Optional` when the entity does not exist yet, or a present `ManagedEntity` otherwise, giving access to `applyStateChange(...)` regardless.
