...
Pass a set of `MessageType` values to `from(...)`, and the one mapper applies to every given identity:
...
public static EventTransformation build() {
    return EventTransformation.from(Set.of(                        // <2>
...
<2> `from(Set)` matches any of the given identities by exact equality, folding all of them up to `1.0.0` with a single registration and a single mapper.

This stays an _exact_ match: every given version is matched by identity, so the chain routes to it in constant time and resolves it without depending on registration order.
Prefer it whenever the versions to cover are known, even a long run produced with code such as `IntStream.range(0, 50).mapToObj(v -> new MessageType(NAME, "0." + v + ".0")).collect(Collectors.toSet())`.

[#_matching_with_a_predicate]
=== Matching with a predicate

This is the exception, not the norm: prefer exact matching (`from(MessageType)`, or `from(Set)` for several known versions), one transformation per version step, and reach for a predicate only when a whole range of historic versions genuinely folds to a single target and the versions cannot be listed up front.
...
Never use one merely to cover a handful of known versions, such as `type -> versions.contains(type.version())`: that runs on every read and the chain cannot optimize it. List the versions in `from(Set)` instead.
====
