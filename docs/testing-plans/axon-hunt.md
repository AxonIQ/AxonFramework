...
| C38 | **REFUTED (F-21).** A stored token never regresses: a proposed token that does not cover the last stored token (strict regression or incomparable) is skipped with a warning instead of written. Measured false for the first store of every claim cycle: the guard is conditional on a field the work package fills only after its first successful store, and a work package is built fresh on every claim, so exactly one unguarded write per claim gets through | safety | documented | `MSG/.../pooled/WorkPackage.java:502-508` "keeping the stored `TrackingToken` monotonic ... is ignored with a warning rather than persisted, so a misbehaving component can never rewind progress on any source."; `:514-522` | The token-store write log for a segment is ever non-monotone under `coversWhenUnwrapped` |
...
| C41 | The broadcast sentinel is a `String` value compared by `equals`/`contains`, so any sequencing policy returning that exact value acquires broadcast semantics | semantics | documented (the constant and its two comparisons) | `MSG/core/sequencing/SequencingPolicy.java:71` `Object BROADCAST = "BROADCAST";`; `MSG/.../pooled/DefaultWorkPackageEventFilter.java:111` `sequenceIdentifiers.contains(SequencingPolicy.BROADCAST)`; `MSG/.../processing/ProcessorEventHandlingComponents.java:194` `SequencingPolicy.BROADCAST.equals(sequenceIdentifier)` | An event whose policy returns the string `BROADCAST` as data is handled once per segment rather than once |
| C42 | A component is invoked exactly once per event across all segments | semantics | documented, and **falsified by the same feature that documents it** | `MSG/.../processing/ProcessorEventHandlingComponents.java:116` "ensuring each event is handled exactly once per component across all segments", against `:190-196` which returns `true` for every segment on a broadcast identifier | One component is invoked more than once for one event within one processor |
| C43 | A pooled processor's `source` and `tokenStore` settings are optional: null or empty means unset, and an unset setting resolves the component by type | semantics | documented | `SPRING/config/SpringCustomizations.java:117-124`; implementation `:161` and `:171` `StringUtils.nonEmptyOrNull(settings.tokenStore()) ? settings.tokenStore() : null` | A setting left unset still demands a named component, so the type-level resolution never runs |
| C44 | A resource whose construction closes over a `ProcessingContext` is never cached under a resource key, unless every possible branch of that context overrides the key | semantics | documented, and **REFUTED at one surviving call site** (finding F-24) | `MSG/core/unitofwork/ProcessingContext.java:111-136` "never use this method for a resource whose construction closes over ... this `ProcessingContext` itself ... every sibling branch that calls afterward receives that same stale instance back - silently operating against the wrong branch" (rule added by PR #4746, which removed the pattern from the three `forContext` factories); violated by `ES/eventstore/InterceptingEventStore.java:106-113`, whose cached `InterceptingEventStoreTransaction` (`:190-195`) intercepts every `appendEvent` with the captured context (`:207-209`) | An event appended through `EventStore.transaction(ctx)` from one per-message branch of a batch is dispatch-intercepted with a sibling branch's context -- observed as message 2's appended event carrying message 1's `correlationId`/`causationId` |
| C47 | A per-event handler span inside a traced streaming batch is a child of the batch span, and its scope is carried on a context *branch*, never written to the shared batch context | semantics | documented | `MSG/eventhandling/tracing/TracingEventHandlingComponent.java:44-50` "its SpanScope is carried on a context branch passed to the delegate"; `MSG/tracing/SpanScope.java:83-86` `addToContext` returns `context.withResource(...)`; `MSG/core/unitofwork/ProcessingContext.java:45-58` `withResource` "Constructs a new ProcessingContext, branching off" | A second event of the same batch is created with the first event's still-open handler scope as its parent -- exactly what a `withResource` that mutates the shared context produces |
| C48 | The tracing event-handling decorator is delivery-transparent: same events, same order, same stream terminations, same phase-action dispatch as the bare delegate; its only additions are completion-handler registrations for span closure | semantics | code-inferred | `MSG/eventhandling/tracing/TracingEventHandlingComponent.java:178-179` (pure delegation through `branchStream`); `MSG/tracing/SpanScopedMessageStream.java:69-71` (`fetchNext` pulls the delegate unchanged inside `scope.within`) | A delivered-payload sequence or phase dispatch sequence that differs between the bare and the decorated run |
| C49 | The `ProcessingLifecycleInterceptor` covers all three dispatch sites, composes without clobbering, survives `forcedSameThreadInvocation()`, never turns a success into a failure, and is absent by default | semantics | documented | `MSG/core/unitofwork/ProcessingLifecycleInterceptor.java:29-59` (three sites, composition, no default install); `MSG/core/unitofwork/UnitOfWorkConfiguration.java:80-88` "the interceptor [is] preserved" across `forcedSameThreadInvocation`; `MSG/core/unitofwork/UnitOfWork.java:479-495` (throwing interceptor logged and swallowed) | An installed pass-through interceptor changes the unit of work's outcome, drops a dispatch site, or is lost by the same-thread switch |
| C51 | An event appended during an active unit of work evolves exactly the co-loaded entities whose `EventCriteria` match its type and resolved tags -- the same filter initial sourcing applies -- and the per-unit-of-work tag cache keys on the event identifier, so same-type payloads with different tag values route independently | semantics | documented | `ES/handler/SimpleEntityLifecycleHandler.java:83-96` (`criteria.matches(event.type().qualifiedName(), tagResolver.resolve(event, context))`); `ES/eventstore/TagResolver.java:67-74` (cache keyed by `event.identifier()`); fix commits `0368916114` + `12e00f4251` (PR #4770). Probed by `simulation/.../scenario/LiveEntityEvolutionRoutingTest`, canary-validated (filter reverted to pre-fix `true \|\|`: 4 of 5 probes red with the cross-contamination signature) | A co-loaded entity's in-memory state after an append differs from the state a fresh reload from the store produces, or an entity evolves on an event whose tags do not match its criteria |

Count: 53 anchored IDs, all filled with real guarantees. 5 are negative results carried as
first-class findings (C9 WEAKER THAN STATED, C12 REFUTED for JPA, C32 DIFFERENT FROM
DOCUMENTED, C38 REFUTED, C42 falsified by the feature that documents it), 2 are documentation
gaps carried as claims (C35 NOT STATED, C39 module absent). 0 unassigned, 0 padded.

C41-C43 and M19-M21 were added while hunting the three features merged for 5.3.0: event handler
checkpointing, the broadcast sequence identifier, and optional pooled source and token store. The
corpus is append-only, so C38's row keeps its original wording and carries the refutation beside it.

C44 was added during the class-of-bug sweep of PR #4746 ("forContext must not cache a context-bound
wrapper"): the rule that PR documented on `ProcessingContext#computeResourceIfAbsent` is itself a
falsifiable claim, and sweeping every resource-caching call in framework main source against it
found one surviving violation (F-24, pinned by `InterceptedTransactionBranchLeakTest`).
...
### A.3 Missing claims M1-M22 (verification of the section 1c list, plus new gaps)
...
| **M19** the anti-rewind guard's scope is never stated | **NEW - CONFIRMED DEFECT (F-21)** | `MSG/.../pooled/WorkPackage.java:507-508` promises "a misbehaving component can never rewind progress on any source" without saying that "the last stored token" means the last token *this work package instance* stored. Searched every Javadoc in `MSG/.../streaming/progress/` (all five files) for `claim`, `first`, `initial`, `scope`: nothing relates the guard to the claim cycle. Nothing says a `WorkPackage` -- and therefore the guard's reference and the strategy instance -- is rebuilt on every claim; the only nearby statement is `:599` "An aborted `WorkPackage` cannot be restarted", in a class a strategy author never sees |
| **M20** no statement that a policy value may collide with the sentinel | **NEW - CONFIRMED DEFECT (F-22)** | `MSG/core/sequencing/SequencingPolicy.java:59-71` documents what `BROADCAST` means and that a `String` is used for cross-JVM `hashCode`/`equals` stability, but never that the value is reserved and must not be returned as data. Searched `ADOC` for `broadcast` (case-insensitive): zero hits across the whole reference guide, so the feature is undocumented outside Javadoc and there is no place a policy author would be warned |
| **M21** "optional when unset" is asymmetric between source and token store | **NEW - CONFIRMED DEFECT (F-23)** | `SB/EventProcessorProperties.java:114` defaults `source` to `null` and `:156` defaults `tokenStore` to the literal `"tokenStore"`, so only the source can ever be unset through Spring Boot properties. `SPRING/config/SpringCustomizations.java:117-124` presents both as one rule. Searched `ADOC` for `token-store` and `source` under `spring-boot-integration`: the properties are not documented at all, so nothing tells an operator that an empty value is the only way to reach the new behaviour |
| **M22** the framework's own batch-span concurrency test coordinates on nothing and runs on a contract-violating context | **NEW - CONFIRMED DEFECT (F-25, in the framework's test suite)** | `MSG-TEST/eventhandling/tracing/TracingEventHandlingComponentTest.java:210-211` barriers on resource label `org.axonframework.messaging.tracing.batchSpan`, but the component's key is `...batchSpanInitializer` (`MSG/eventhandling/tracing/TracingEventHandlingComponent.java:93-94`) and the lookup goes through `computeResourceIfAbsent`, not `getResource`, so the barrier never fires. `MSG-TEST/core/unitofwork/StubProcessingContext.java:157-161` implements `withResource` as `resources.put(key, resource); return this;`, violating the branching contract of `MSG/core/unitofwork/ProcessingContext.java:45-58` and leaking every branch-scoped handler span onto the shared batch context. Measured: 6/40 failures of `TracingEventHandlingComponentTest$BatchSpan` on this machine |
| **M27** nothing states a stored snapshot's payload is insulated from later mutation of the entity it was taken from | **NEW - CONFIRMED DEFECT (F-33)** | `ES/handler/SnapshottingEntityLifecycleHandler.java:203` stores the live entity reference (`new Snapshot(position, ..., entity, ...)`), and `:217` returns the stored payload reference as-is when `entityType.isInstance(snapshot.payload())`. `ES/snapshot/inmemory/InMemorySnapshotStore.java:56-58` keeps the `Snapshot` record by reference. Neither `SnapshotStore`'s Javadoc nor the handler's states who must copy; with a mutable entity (void `@EventSourcingHandler`s evolve in place) a live append after sourcing, or after a snapshot restore, silently rewrites the stored snapshot while its position stays put, and the next load applies the same event twice. (M22-M26 are held by parallel hunt agents.) |
...
### A.4 Claims C55-C59, added while hunting the 5.3.0-RC1 tracing rework (#4713)

C44-C54 and M22-M23 are held by the parallel RC1 hunts and are not listed here.

| # | Claim | Kind | Confidence | Anchor | Falsified by |
|---|---|---|---|---|---|
| C55 | A handler span continues its dispatch span's trace through context propagated in message metadata; a streaming handler span parents under the batch span and links back to the publisher | semantics | documented | `MSG/tracing/SpanFactory.java:76-105`; `MSG/eventhandling/tracing/TracingEventHandlingComponent.java:52-64` | a history in which the handler span's recorded parent is not the dispatch span, or the streaming handler span's link does not resolve the publish span |
| C56 | Every span a structured operation opens ends exactly once, on both the value and the throw path, with failures recorded before the scope closes | semantics | documented | `MSG/tracing/Span.java:144-152` (branch), `:188-193` (branchAsync), `:246-251` (branchStream) | any span recorded started but never ended after the operation's result terminated, or a failed operation whose span carries no exception |
| C57 | A branch-scoped span's children read back exactly the branch's scope, however many other operations start and finish elsewhere on the root in the meantime | semantics | documented | `MSG/tracing/SpanScope.java:59-64` | a child span created with a branched context whose recorded parent is another operation's span |
| C58 | Re-entering the scope around every pull lets provider context-capture observe the operation's span when lazy work starts | semantics | documented | `MSG/tracing/SpanScopedMessageStream.java:27-34` | a pull of a branch-scoped stream during which the provider's current-scope state is absent or another span's |
| C59 | A reactive handler result is subscribed with ThreadLocal context capture, so thread-bound state present at subscription reaches the operators downstream | semantics | documented | `MSG/core/FluxUtils.java:76-80`, `MSG/core/MonoUtils.java:42-46` | a subscription during which a registered ThreadLocalAccessor's value does not appear in the Reactor ContextView |

Coverage: C55 by `simulation/.../tracing/TracingTraceContinuityTest` (end-to-end span tree); C56 by
the failure-path cases in both tracing test classes; C57 by
`TracingScopeLifecycleTest$ConcurrentBranchScopes` (two threads, shared production context,
root-churn thread); C58 by the same arm's gated-future pull probe (canary-validated, one escape
recorded in `formal/CANARIES.md` 2026-08-04); C59 by
`TracingScopeLifecycleTest$ReactiveThreadLocalCapture` (canary-validated). All five VERIFIED on
5.3.0-RC1; no refutation, so no F-number was assigned.
