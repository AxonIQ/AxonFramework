...
| M9 Axon Server reconnect/redelivery semantics | **CONFIRMED-GAP (module absent)** | No `*axonserver*` or `*connector*` directory exists at depth 3 in this tree. The only in-tree references are Javadoc examples and a thread-name constant |
...
| `AxonServerTestInfrastructure` | referenced only in Javadoc at `AbstractIT.java:55` | not implemented |
...
| No Axon Server anywhere | see B.3 |
